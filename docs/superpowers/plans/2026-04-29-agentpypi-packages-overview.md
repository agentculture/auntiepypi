# agentpypi v0.1.0 — `packages overview` Implementation Plan
...
**Architecture:** Per-dimension plugin under `agentpypi/_rubric/`, parallel to the existing `agentpypi/_probes/`. Read-only, stdlib-only HTTP against pypi.org JSON + pypistats.org. Section JSON shape standardises on `{category, title, light, fields}` (breaking change vs v0.0.1's `{name, summary, items}` — only this PR has both shapes in flight).
...
**Spec:** `docs/superpowers/specs/2026-04-29-agentpypi-packages-overview-design.md`.
...
- `agentpypi/_packages_config.py` — load `[tool.agentpypi].packages` from `pyproject.toml` (walks up from CWD).
- `agentpypi/_rubric/__init__.py` — `DIMENSIONS` registry.
- `agentpypi/_rubric/_dimension.py` — `Score`, `DimensionResult`, `Dimension` types.
- `agentpypi/_rubric/_runtime.py` — `roll_up`, `evaluate_package`, deep-dive renderer.
- `agentpypi/_rubric/_fetch.py` — `get_json`, `FetchError`.
- `agentpypi/_rubric/_sources.py` — `fetch_pypi`, `fetch_pypistats`.
- `agentpypi/_rubric/recency.py`
- `agentpypi/_rubric/cadence.py`
- `agentpypi/_rubric/downloads.py`
- `agentpypi/_rubric/lifecycle.py`
- `agentpypi/_rubric/distribution.py`
- `agentpypi/_rubric/metadata.py`
- `agentpypi/_rubric/versioning.py`
- `agentpypi/cli/_commands/_packages/__init__.py`
- `agentpypi/cli/_commands/_packages/overview.py`
...
- `pyproject.toml` — version bump 0.0.2 → 0.1.0; add `[tool.agentpypi]`; bump coverage gate to 95; register `live` marker.
- `agentpypi/cli/__init__.py` — register the `packages` subparser.
- `agentpypi/cli/_commands/overview.py` — promote to composite (packages + servers) with new section shape.
- `agentpypi/cli/_commands/learn.py` — drop `online …` from `planned`; add packages overview to commands.
- `agentpypi/explain/catalog.py` — replace `("online",)` with `("packages",)` and `("packages", "overview")`; update `_ROOT`.
...
- Modify: `agentpypi/__main__.py` (add pragma)
- Modify: `agentpypi/cli/__init__.py:65-72` (add pragma)
- Modify: `agentpypi/_probes/_runtime.py:74-80` (add pragma; defensive OSError fallback)
...
Edit `agentpypi/cli/__init__.py`, the `except Exception` block at lines 65-72. Replace:
...
In `agentpypi/_probes/_runtime.py`, the `except OSError as err` block (lines 74-86). Add `# pragma: no cover` to the `except` line:
...
Read `agentpypi/cli/_commands/whoami.py` lines 45-61. These are pip-conf parsing branches. Add a test at `tests/test_whoami.py`:

```python
def test_whoami_reads_pip_conf(tmp_path, monkeypatch):
    """Cover the pip.conf parsing branch (currently uncovered)."""
    from agentpypi.cli._commands.whoami import cmd_whoami
...
Read `agentpypi/cli/_commands/doctor.py` lines 42-44, 60-63, 83, 85, 139, 141-142. These are `--fix` failure paths and the rendering of `down` items. Add a parametrised test at `tests/test_doctor.py`:
...
def test_doctor_renders_down_with_detail(monkeypatch, capsys):
    ...
    from argparse import Namespace
    from agentpypi.cli._commands.doctor import cmd_doctor

    fake = [
        {"name": "devpi", "port": 3141, "url": "http://x", "status": "down", "detail": "http 500"},
        {"name": "pypiserver", "port": 8080, "url": "http://y", "status": "up"},
    ]
    monkeypatch.setattr("agentpypi.cli._commands.doctor.probe_status",
...
```bash
uv run pytest -n auto --cov=agentpypi --cov-report=term -q
```
...
```bash
git add pyproject.toml agentpypi/cli/__init__.py agentpypi/_probes/_runtime.py tests/test_whoami.py tests/test_doctor.py
...
## Task 4: Config loader — `[tool.agentpypi].packages`

**Files:**

- Create: `agentpypi/_packages_config.py`
...
Expected: collection error / `ModuleNotFoundError: agentpypi._packages_config`.
    ...
    while True:
        ...
        if candidate.exists():
            try:
                ...
                if "tool" in data and "agentpypi" in data["tool"]:
                    return candidate
...
def load_package_names(start: Path | None = None) -> list[str]:
    ...
    if found is None:
        raise ConfigError("no pyproject.toml with [tool.agentpypi] found")
    with found.open("rb") as f:
        data = tomllib.load(f)
    table = data.get("tool", {}).get("agentpypi", {})
    if "packages" not in table:
        raise ConfigError("no [tool.agentpypi].packages key")
    pkgs = table["packages"]
...
```bash
git add agentpypi/_packages_config.py tests/test_packages_config.py
git commit -m "feat(packages): config loader for [tool.agentpypi].packages"
```
...
- Create: `agentpypi/_rubric/__init__.py` (placeholder; later tasks fill `DIMENSIONS`)
- Create: `agentpypi/_rubric/_dimension.py`
- Create: `agentpypi/_rubric/_runtime.py`
...
Expected: collection error / `ModuleNotFoundError: agentpypi._rubric`.

- [ ] **Step 3: Create `_rubric/__init__.py` (empty placeholder)**

Create `agentpypi/_rubric/__init__.py`:
...
```bash
git add agentpypi/_rubric/ tests/test_rubric_runtime.py
...
- Create: `agentpypi/_rubric/_fetch.py`
...
```bash
git add agentpypi/_rubric/_fetch.py tests/test_rubric_fetch.py
...
- Create: `agentpypi/_rubric/_sources.py`
...
from agentpypi._rubric._fetch import get_json
...
```bash
git add agentpypi/_rubric/_sources.py tests/test_rubric_sources.py
...
- Create: `agentpypi/_rubric/recency.py`
...
```bash
git add agentpypi/_rubric/recency.py tests/test_rubric_recency.py
...
- Create: `agentpypi/_rubric/cadence.py`
...
```bash
git add agentpypi/_rubric/cadence.py tests/test_rubric_cadence.py
...
- Create: `agentpypi/_rubric/downloads.py`
...
```bash
git add agentpypi/_rubric/downloads.py tests/test_rubric_downloads.py
...
- Create: `agentpypi/_rubric/lifecycle.py`
...
```bash
git add agentpypi/_rubric/lifecycle.py tests/test_rubric_lifecycle.py
...
- Create: `agentpypi/_rubric/distribution.py`
...
```bash
git add agentpypi/_rubric/distribution.py tests/test_rubric_distribution.py
...
- Create: `agentpypi/_rubric/metadata.py`
...
```bash
git add agentpypi/_rubric/metadata.py tests/test_rubric_metadata.py
...
- Create: `agentpypi/_rubric/versioning.py`
...
```bash
git add agentpypi/_rubric/versioning.py tests/test_rubric_versioning.py
...
- Modify: `agentpypi/_rubric/__init__.py`
- Modify: `agentpypi/_rubric/_runtime.py` — add `evaluate_package`
...
Replace `agentpypi/_rubric/__init__.py`:
...
Append to `agentpypi/_rubric/_runtime.py`:
...
```bash
git add agentpypi/_rubric/__init__.py agentpypi/_rubric/_runtime.py tests/test_rubric_runtime.py
git commit -m "feat(rubric): wire DIMENSIONS registry + evaluate_package"
```

---

## Task 16: `agentpypi packages overview` CLI verb

**Files:**

- Create: `agentpypi/cli/_commands/_packages/__init__.py`
- Create: `agentpypi/cli/_commands/_packages/overview.py`
- Modify: `agentpypi/cli/__init__.py:18-23`, `:47-53`
...
Edit `agentpypi/cli/__init__.py`. After line 23 (the existing `from agentpypi.cli._commands import whoami as _whoami_cmd`), add:
...
```bash
git add agentpypi/cli/_commands/_packages/ agentpypi/cli/__init__.py tests/test_cli_packages.py
git commit -m "feat(cli): agentpypi packages overview [PKG]"
```
...
- Modify: `agentpypi/cli/_commands/overview.py`
...
def _composite_no_arg(json_mode: bool) -> int:
    ...
    try:
        ...
        names = []
        emit_diagnostic(
            "warning: no [tool.agentpypi].packages configured; "
...
The test must still pass without configured packages (since `tmp_path` won't have `[tool.agentpypi]`). The `_composite_no_arg` path warns and emits servers-only — that's fine.

If existing tests assert on `subject == "agentpypi overview"`, that still holds.
...
```bash
git add agentpypi/cli/_commands/overview.py tests/test_overview.py tests/test_cli_overview_composite.py
git commit -m "feat(cli): promote agentpypi overview to packages+servers composite"
```
...
- Modify: `agentpypi/explain/catalog.py`
...
- [ ] **Step 3: Edit `agentpypi/explain/catalog.py`**
...
```bash
git add agentpypi/explain/catalog.py tests/test_cli.py
...
- Modify: `agentpypi/cli/_commands/learn.py`
...
Also add `agentpypi packages overview` to the Commands block (insert after the `agentpypi overview` block, before `agentpypi doctor`):
...
```bash
git add agentpypi/cli/_commands/learn.py tests/test_cli.py
...
- Update the existing `agentpypi overview [TARGET]` description to say "composite of packages + local server probes; with TARGET drills into a server flavor or configured package".
- Add `agentpypi packages overview [PKG] [--json]` to the active list.
...
## Task 21: Configure example `[tool.agentpypi].packages`
...
```toml
[tool.agentpypi]
# Packages this repo cares about for `agentpypi packages overview`.
...
```bash
uv run agentpypi packages overview --json | head -50
```
...
```bash
git add pyproject.toml
git commit -m "config: [tool.agentpypi].packages with sibling list"
```
  ...
  smoke-source:
    ...
    steps:
      ...
      - name: Smoke — version + packages overview against pypi.org
        run: |
          /tmp/smoke/bin/agentpypi --version
          /tmp/smoke/bin/agentpypi packages overview requests --json | python -m json.tool
```
    ...
    steps:
      ...
      - name: Wait for new version on pypi.org
        run: |
          ...
          echo "Waiting for agentpypi==$VERSION on pypi.org…"
          for i in $(seq 1 18); do
            if curl -sf "https://pypi.org/pypi/agentpypi/$VERSION/json" -o /dev/null; then
              echo "Available."
              exit 0
            fi
            sleep 5
          done
          echo "::error::Timed out waiting for agentpypi==$VERSION on pypi.org"
          exit 1
      - name: Install from pypi.org and smoke
        run: |
          ...
          /tmp/postsmoke/bin/pip install --quiet "agentpypi==$VERSION"
          /tmp/postsmoke/bin/agentpypi --version
          /tmp/postsmoke/bin/agentpypi packages overview requests --json | python -m json.tool
```
...
uv run agentpypi packages overview --json | head -30
uv run agentpypi packages overview requests --json | head -50
uv run agentpypi overview --json | head -30
uv run agentpypi explain packages
uv run agentpypi explain packages overview
uv run agentpypi learn --json | head -40
```
...
```bash
(cd ../steward && uv run steward doctor --scope self ../agentpypi) || true
```
...
Invoke the agentpypi `pr-review` skill or run `gh pr create` directly:

```bash
gh pr create --title "v0.1.0: packages overview (informational, not gating)" --body "$(cat <<'EOF'
## Summary

- Ships the `packages` noun: `agentpypi packages overview [PKG]` — read-only PyPI maturity dashboard / per-package deep-dive.
- Promotes top-level `agentpypi overview` to a composite of `packages` + `servers` sections; section JSON shape standardises on `{category, title, light, fields}`.
...
- [x] `uv run agentpypi packages overview --json` round-trips through `jq`
- [x] `uv run agentpypi overview --json` shows mixed `category` sections
- [x] `uv run agentpypi explain packages overview` returns non-empty markdown
- [x] `uv run agentpypi learn --json | jq .planned` no longer mentions `online`
...
- [x] Static config in `[tool.agentpypi].packages` — Tasks 4, 21
