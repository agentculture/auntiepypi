# agentpypi v0.1.0 — `packages overview` design
...
- `agentpypi packages overview [PKG]` — read-only multi-package roll-up
  (no arg) or per-package deep-dive (with arg) against pypi.org's public
  JSON API plus pypistats.org for download counts.
- `agentpypi overview` is **promoted** to a composite: it now emits both
...
| Source of truth for "my packages" | Static list of strings in `pyproject.toml` `[tool.agentpypi].packages` |
...
| Coverage gate | 95% on the `agentpypi/` package, with documented `pragma: no cover` allowlist |
| Pre/post-deploy checks | Smoke jobs in `publish.yml`: build-and-run before upload; install-and-run after upload |
| `agentpypi overview` shape | Composite of `packages` + `servers` sections, tagged by `category` key |

## Surface

```text
agentpypi packages overview [PKG] [--json]
agentpypi overview [TARGET] [--json]            # composite, promoted
```

### `agentpypi packages overview` (no arg)

Roll-up over `pyproject.toml [tool.agentpypi].packages`. One section per
...
[tool.agentpypi].packages` on stderr and a `hint:` pointing to the config
block.

### `agentpypi packages overview <pkg>`
...
### `agentpypi overview` (composite, promoted)
...
- `1` — user-input error (missing/empty `[tool.agentpypi.packages]`
...
```json
{
  "subject": "packages" | "<pkg-name>" | "agentpypi",
  "sections": [
    {
...
composite `agentpypi overview` emits mixed.
...
```toml
[tool.agentpypi]
packages = ["afi-cli", "shushu", "ghafi", "steward", "agentpypi"]
```
...
- **Per-fetch:** 5 s timeout, no retries, custom UA
  `agentpypi/<__version__> (+https://github.com/agentculture/agentpypi)`,
...
| `publish.yml` pre-upload | install built wheel into clean venv → run `agentpypi --version` and `agentpypi packages overview requests --json`; aborts upload on failure |
...
Gate moves from 60% to **95%** on the `agentpypi/` package, configured
in `pyproject.toml`:
...
- **`agentpypi/explain/catalog.py`** — drop the `("online",)`
  placeholder; add `("packages",)` and `("packages", "overview")`. Root
  entry moves `agentpypi packages overview` from "Planned" into "Verbs"
  and updates the noun list. `local` stays in "Planned".
- **`agentpypi/cli/_commands/learn.py`** — JSON `planned` array drops
...
- **`README.md`** — opening paragraph adopts the Statement of purpose
  ...
  `agentpypi packages overview` output.
...
- v0.2.0 introduces the `servers` noun. `agentpypi servers overview` is
...
uv run agentpypi packages overview --json | jq .
uv run agentpypi packages overview requests --json | jq .
uv run agentpypi overview --json | jq '.sections | group_by(.category) | map({category: .[0].category, count: length})'
uv run agentpypi explain packages
uv run agentpypi explain packages overview
uv run agentpypi learn --json | jq '.planned'
