...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope auntiepypi
...
scope:** the default personal scope is **private** (`--scope auntiepypi
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `auntiepypi`). Private records are served only to a recall in the same
